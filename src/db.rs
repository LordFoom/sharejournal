...
pub fn buy_share(share: &Share, db: &Database)->Result<(), Report>{
    ...
    let conn= db.connection();
    println!("{:?}", share);
    conn.execute("INSERT INTO sharejournal (share_code, share_name, buy_price, buy_date)
    VALUES (?1, ?2, ?3, ?4", params![&share.code, &share.name, &share.buy_price, &share.buy_date])?;
    Ok(())
}
...
mod tests{
    ...
    pub fn test_buy_share() {
        ...
        let share = Share::share_to_buy("Test Share Company".to_string(),
                                        "TST".to_string(),
                                        12.34,
                                        date);
        let db = test_db();
